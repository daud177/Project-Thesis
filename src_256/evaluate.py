...
def run_simulation(env, policy, episodes):
    ...
    for episode in range(episodes):
        ...
        #os.chdir('/home/faps/CEILing/CEILing256/data/' + config["task"])
...
def main(config):
    try:
        ...
        if not os.path.exists('/home/faps/CEILing/CEILing256/data/'+ config["task"] + '/process_information_evaluate.csv'): # create a directory to sort the results of process
                os.chdir('/home/faps/CEILing/CEILing256/data/' + config["task"])
                with open('process_information_evaluate.csv', mode='a',newline='') as results_file:
                    results_file_writer = csv.writer(results_file, delimiter=';', quotechar='"', quoting=csv.QUOTE_NONNUMERIC)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        help="options: pretraining, cloning_1, cloning_2, cloning_4, cloning_6, cloning_10, cloning_25, cloning_50, cloning_75, cloning_100, evaluative, dagger, iwr, ceiling_25, ceiling_50, ceiling_75, ceiling_full, ceiling_partial",
    )
    parser.add_argument(
        "-t",
        "--task",
