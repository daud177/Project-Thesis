...
class CustomEnv:
    def __init__(self, config):
        ...
        self.reward = 0
        self.done = False
        self.path = '/home/faps/CEILing256/Test_Images'
    ...
    def image_resize_transpose(self, frame):
        self.frame = cv2.resize(frame, (256, 256))
    ...
    def reset(self):
        ...
        if self.gripper_last_state != 1: #0.9?
            ...
            #self.control_wsg50.release(50,50) # for socket
            pass
    ...
    def resetRoboPos(self):
        ...
        try:
            ...
            #self.move_client_kuka.move([0, 0.4475, 0, -1.76418, 0, 0.9070, 0],[0.1])  # for socket picking position
        except:
            self.move_client_kuka.disconnect()
            input('Restart MatlabToolboxServer application in smartpad and press enter key \n') 
            self.move_client_kuka.connect()
