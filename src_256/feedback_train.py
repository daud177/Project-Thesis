...
def run_env_simulation(env, policy, replay_memory, metrics_logger, config):
    ...
    for episode in range(config["episodes"]):
        ...
        if episode == 24 or episode == 49 or episode == 74:
            ...
            file_name = "/home/faps/CEILing/CEILing256/data/" + config["task"] + "/" + config["feedback_type"] + str(episode) + "_policy.pt"
...
def main(config):
    ...
    try: 
        ...
        if not os.path.exists('/home/faps/CEILing/CEILing256/data/'+ config["task"] + '/process_information_feedback_train.csv'): # create a directory to sort the results of process
                os.chdir('/home/faps/CEILing/CEILing256/data/' + config["task"])
                with open('process_information_feedback_train.csv', mode='a',newline='') as results_file:
                    results_file_writer = csv.writer(results_file, delimiter=';', quotechar='"', quoting=csv.QUOTE_NONNUMERIC)
        ...
        time.sleep(60)
        stop_flag.set()
        file_name = "/home/faps/CEILing/CEILing256/data/" + config["task"] + "/" + config["feedback_type"] + "_policy.pt"
        torch.save(policy.state_dict(), file_name)
        cap.release()#
