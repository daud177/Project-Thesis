...
class Policy(nn.Module):
    def __init__(self, config, rank, world_size):
        ...
        lstm_dim = 4096 + config["proprio_dim"]  # Adjust LSTM input dimension based on visual encoding and proprioceptive input
    ...
    def forward_step(self, camera_obs, proprio_obs, lstm_state):
        ...
        # Flatten the visual encoding output
        vis_encoding = torch.flatten(vis_encoding, start_dim=1)

        # Concatenate visual encoding and proprioceptive inputs
...
def binary_gripper(gripper_action):
    return 0.9 if gripper_action >= 0.0 else -0.9
