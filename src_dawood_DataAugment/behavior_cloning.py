...
from torch.multiprocessing import spawn, Pipe
...
def augment_image(image):
    ...
    try:
        for i in range(image.shape[0]):
            ...
            img = cv2.resize(np.transpose(img, (1, 2, 0)), (256, 256), interpolation=cv2.INTER_LINEAR)
...
def train_step(policy, step, replay_memory, config, len, rank):
    dataLoader_local = get_data_loader(replay_memory, batch_size=config["batch_size"], len_dataset=len, augment=True)   # get dataloader in each train step
...
def get_data_loader(dataset, batch_size, len_dataset, augment=False):
    ...
    def collate_fn(batch):
        camera_batch = torch.stack(
            [torch.from_numpy(augment_image(item[0].cpu().numpy())).float().to(item[0].device) if augment else item[0]
             for item in batch], dim=1)
        proprio_batch = torch.stack([item[1] for item in batch], dim=1)
        action_batch = torch.stack([item[2] for item in batch], dim=1)
        feedback_batch = torch.stack([item[3] for item in batch], dim=1)
        return camera_batch, proprio_batch, action_batch, feedback_batch
...
def main(rank, config, world_size, conn):
    ...
    dataset_name = f"/demos_{config['feedback_type'].split('_')[1]}.dat"
    replay_memory = torch.load(f"data/{config['task']}{dataset_name}")
    ...
    if rank == 0:
        file_name = f"data/{config['task']}/{config['feedback_type']}_policy.pt"
    ...
    conn.send(final_loss)
    conn.close()


def objective(trial):
    config = {
        "task": "dawood_test_200",
    ...
    spawn(main, args=(config, world_size, child_conn), nprocs=world_size, join=True)
    
    final_loss = parent_conn.recv()  # Receive the result from the child process
    return final_loss
...
if __name__ == "__main__":
    ...
    if args.optimize:
        ...
        world_size = 2  # Number of GPUs
        parent_conn, child_conn = Pipe()
        spawn(main, args=(config_defaults, world_size, child_conn), nprocs=world_size, join=True)
